...
class seq2seq_attention_model:
    ...
    def inference(self):
        ...
        # 2.encoder with GRU
        # 2.1 forward gru
        hidden_state_forward_list = self.gru_forward_word_level(self.embedded_words)  # a list,length is sentence_length, each element is [batch_size,hidden_size]
        # 2.2 backward gru
        hidden_state_backward_list = self.gru_forward_word_level(self.embedded_words,reverse=True)  # a list,length is sentence_length, each element is [batch_size*num_sentences,hidden_size]
        ...
        decoder_inputs=[tf.concat([h_forward,h_backward],axis=1) for h_forward,h_backward in zip(hidden_state_forward_list,hidden_state_backward_list)]#list,len:sent_len,e:[batch_size,hidden_size*2]
        ...
        #rnn_decoder_with_attention(decoder_inputs, initial_state, cell, loop_function,attention_states,scope=None):
            ...
            #output:?
        decoder_inputs=tf.stack(decoder_inputs,axis=1) #shape:[batch_size,sentence_length,hidden_size*2]
        initial_state=tf.reduce_sum(decoder_inputs,axis=1) #[batch_size,hidden_size*2] #TODO NEED TO TEST WHICH ONE IS BETTER: SUM UP OR USE LAST HIDDEN STATE==>similiarity.
        ...
        attention_states=decoder_inputs #self.embedded_words #[None, self.sequence_length, self.embed_size]
        y=tf.nn.embedding_lookup(self.Embedding_label,self.decoder_input)
        y_splitted = tf.split(y, self.decoder_sent_length,axis=1)  # it is a list,length is decoder_sent_length, each element is [batch_size,1,embed_size]
        y_squeezed = [tf.squeeze(x, axis=1) for x in y_splitted]  # it is a list,length is decoder_sent_length, each element is [batch_size,embed_size]
        outputs, final_state=rnn_decoder_with_attention(y_squeezed, initial_state, cell, loop_function, attention_states, scope=None) # A list.length:decoder_sent_length.each element is:[batch_size x output_size]
    ...
    def gru_forward_word_level(self, embedded_words,reverse=False):
        """
        ...
        for time_step, Xt in enumerate(embedded_words_squeeze):  # Xt: [batch_size,embed_size]
            h_t = self.gru_cell(Xt,h_t) #h_t:[batch_size,embed_size]<------Xt:[batch_size,embed_size];h_t:[batch_size,embed_size]
            h_t_list.append(h_t)
        if reverse:
            h_t_list.reverse()
    ...
    def instantiate_weights(self):
        ...
        with tf.name_scope("gru_weights_decoder"):
            ...
            self.U_h_decoder = tf.get_variable("U_h_decoder", shape=[self.embed_size*2, self.hidden_size*2], initializer=self.initializer)
...
def test():
    ...
    with tf.Session() as sess:
        ...
        for i in range(1658):
            ...
            label = np.array([2,3,4,5,6],dtype=np.int32) #[2,3,4,5,6]
            decoder_input=np.array([[0,2,3,4,5,6]],dtype=np.int32) #[0,2,3,4,5,6]
            input_y_label=np.array([[2,3,4,5,6,1]],dtype=np.int32) #[2,3,4,5,6,1]
            ...
            print(i,"loss:", loss, "acc:", acc, "input_y_label:", input_y_label, "prediction:", predict)
#test()