...
class BiLstmTextRelationTwoRNN:
    ...
    def instantiate_weights(self):
        ...
        with tf.name_scope("embedding"): # embedding matrix
            ...
            self.W = tf.get_variable("W",shape=[self.hidden_size*2, self.hidden_size*2],initializer=self.initializer) #[self.hidden_size*2, self.hidden_size*2]
            self.b = tf.get_variable("b",shape=[self.hidden_size*2])       #[label_size]

            self.W_projection = tf.get_variable("W_projection",shape=[self.batch_size, self.num_classes],initializer=self.initializer) #[embed_size,label_size]
    ...
    def bi_LSTM_layer(self,sentence):
        # 1.get emebedding of words in the sentence
        embedded_words = tf.nn.embedding_lookup(self.Embedding,sentence)  # shape:[None,sentence_length,embed_size]
        # 2. Bi-lstm layer
