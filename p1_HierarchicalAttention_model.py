...
class HierarchicalAttention:
    def __init__(self,num_classes, learning_rate, batch_size, decay_steps, decay_rate,sequence_length,num_sentences,vocab_size,embed_size,
                 hidden_size,is_training,need_sentence_level_attention_encoder_flag=True,multi_label_flag=False,initializer=tf.random_normal_initializer(stddev=0.01)):
        ...
        self.sequence_length = self.sequence_length/self.num_sentences #TODO
    ...
    def inference(self):
        ...
        with tf.name_scope("output"): #TODO
    ...
    def gru_forward_word_level(self, embedded_words):
        ...
        h_t = tf.ones((self.batch_size*self.num_sentences, self.hidden_size)) # h_t =int(tf.get_shape(embedded_words_squeeze[0])[0]) # tf.ones([self.batch_size*self.num_sentences, self.hidden_size]) # [batch_size*num_sentences,embed_size]
        h_t_forward_list = []
    ...
    def gru_backward_word_level(self, embedded_words):
        ...
        h_t = tf.ones((self.batch_size*self.num_sentences, self.hidden_size)) #TODO
        h_t_backward_list = []
...
def test():
    ...
    textRNN=HierarchicalAttention(num_classes, learning_rate, batch_size, decay_steps, decay_rate,sequence_length,num_sentences,vocab_size,embed_size,
                 hidden_size,is_training)
    with tf.Session() as sess:
      sess.run(tf.global_variables_initializer())
      for i in range(100):
          #input_x should be:[batch_size, num_sentences,self.sequence_length]
          input_x=np.zeros((batch_size,num_sentences,sequence_length))
          ...
          loss,acc,predict,W_projection_value,_=sess.run([textRNN.loss_val,textRNN.accuracy,textRNN.predictions,textRNN.W_projection,textRNN.train_op],
