...
class Decoder(BaseClass):
    def __init__(self,d_model,d_k,d_v,sequence_length,h,batch_size,Q,K_s,decoder_inputs,K_v_encoder,decoder_sent_length,num_layer=6,type='decoder',is_training=True):
        """
        :param d_model:
        :param d_k:
        :param d_v:
        ...
        :param Q:
        :param K_s:
        :param decoder_inputs: shape:[batch_size*sequence_length,embed_size]. it is decoder inputs.
    ...
    def decoder_fn(self):
        ...
        if self.is_training:
            ...
            raise ValueError #TODO need think about how to constructe a mask with know postion when testing
        for layer_index in range(self.num_layer):
            Q,K_s=self.decoder_single_layer(Q, K_s, layer_index, mask)
        return Q,K_s
        print("decoder.decoder_fn.ended.Q:",Q," ;K_s:",K_s)

    def decoder_single_layer(self,Q,K_s,layer_index,mask):
        """
        ...
        multi_head_attention_output=self.sub_layer_multi_head_attention(layer_index,Q,K_s,self.type,mask=mask) #[batch_size*sequence_length,d_model]
        #1.2 use LayerNorm(x+Sublayer(x)). all dimension=512.
        ##################norm_residual_output=self.sub_layer_layer_norm_residual_connection(K_s,multi_head_attention_output) #[batch_size,sequence_length,d_model]
        ...
        attention_enc_dec=AttentionEncoderDecoder(self.d_model,self.d_k,self.d_v,self.sequence_length,self.h,self.batch_size,
                                                  multi_head_attention_output,self.K_v_encoder,layer_index,self.decoder_sent_length) #TODO norm_residual_output
        ...
        ###############norm_residual_output2= self.sub_layer_layer_norm_residual_connection(norm_residual_output,attention_enc_dec_output)  # [batch_size,sequence_length,d_model]
        ###############print("decoder.2.2.norm_residual_output2:",norm_residual_output2)

        # 3.1 the second is position-wise fully connected feed-forward network.
        postion_wise_feed_forward_output=self.sub_layer_postion_wise_feed_forward(attention_enc_dec_output,layer_index,self.type) #TODO norm_residual_output2.[batch_size,sequence_length,d_model]
        ...
        ####################norm_residual_output_sublayer3= self.sub_layer_layer_norm_residual_connection(norm_residual_output2,postion_wise_feed_forward_output)
        ####################print("decoder.3.2.norm_residual_output_sublayer3:",norm_residual_output_sublayer3)
        return postion_wise_feed_forward_output,postion_wise_feed_forward_output #TODO norm_residual_output_sublayer3,norm_residual_output_sublayer3
...
def init():
    ...
    decoder = Decoder(d_model, d_k, d_v, sequence_length, h, batch_size, Q, K_s, embedded_words, K_v_encoder,decoder_sent_length)
